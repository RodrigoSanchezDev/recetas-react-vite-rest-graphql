...
const Footer = () => {
  ...
  const footerLinks = {
    recetas: [
      { name: 'Explorar Recetas', path: '/recetas' },
      { name: 'Crear Receta', path: '/crear-receta' },
      { name: 'Categorías', path: '/recetas?filter=all' },
    ],
    compañia: [
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="col-span-1">
            <div className="flex items-center space-x-2 mb-4">
              ...
              <span className="text-xl font-semibold text-white">RecetasHub</span>
            </div>
            <p className="text-sm text-amber-300 leading-relaxed">
              Tu plataforma de confianza para descubrir y compartir recetas deliciosas.
            </p>
              ...
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link.name}>
                    <Link
                      to={link.path}
                      className="text-sm text-amber-300 hover:text-white transition-colors duration-200"
                    >
                      {link.name}
                    </Link>
                  </li>
        ...
        <div className="border-t border-amber-800 mt-12 pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <p className="text-sm text-amber-400">
              © {currentYear} RecetasHub. Todos los derechos reservados.
            </p>
            <p className="text-sm text-amber-400">
